class RecruiterController < ApplicationController
  ...
  def index_accepted
    ...
    if current_user.has_role? "recruiter_coordinator"
      ...
      @provinces_count_cibubur = @profiles.where("accepted_location IS NOT NULL and accepted_location = 'Cibubur'").count(:all, :group => :province, :order => 'province')
      @schools_count_cibubur = @profiles.where("accepted_location IS NOT NULL and accepted_location = 'Cibubur'").count(:all, :group => :school, :order => 'school')
      @genders_count_cibubur = @profiles.where("accepted_location IS NOT NULL and accepted_location = 'Cibubur'").count(:all, :group => :gender, :order => 'gender')
      
      @provinces_count_bukittinggi = @profiles.where("accepted_location IS NOT NULL and accepted_location = 'Bukit Tinggi'").count(:all, :group => :province, :order => 'province')
      @schools_count_bukittinggi = @profiles.where("accepted_location IS NOT NULL and accepted_location = 'Bukit Tinggi'").count(:all, :group => :school, :order => 'school')
      @genders_count_bukittinggi = @profiles.where("accepted_location IS NOT NULL and accepted_location = 'Bukit Tinggi'").count(:all, :group => :gender, :order => 'gender')
      
      @provinces_count_both = @profiles.where("accepted_location IS NOT NULL and accepted_location = 'Cibubur atau Bukit Tinggi'").count(:all, :group => :province, :order => 'province')
      @schools_count_both = @profiles.where("accepted_location IS NOT NULL and accepted_location = 'Cibubur atau Bukit Tinggi'").count(:all, :group => :school, :order => 'school')
      @genders_count_both = @profiles.where("accepted_location IS NOT NULL and accepted_location = 'Cibubur atau Bukit Tinggi'").count(:all, :group => :gender, :order => 'gender')
      
      @provinces_count_none = @profiles.where("accepted_location IS NOT NULL and accepted_location = 'Tidak bisa mengikuti'").count(:all, :group => :province, :order => 'province')
      ...
      @recently_added_profiles = @profiles.order('updated_at DESC').limit(10)
